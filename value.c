...
struct cee_array * cee_json_to_array (struct cee_json * p) {
  if (p->t == is_array) {
...
struct cee_json * cee_json_number (double d) {
  ...
  struct cee_tagged * t = cee_tagged (is_number, p);
  return (struct cee_json *)t;
}

struct cee_json * cee_json_string(struct cee_str *s) {
  struct cee_tagged * t = cee_tagged (is_string, s);
  return (struct cee_json *)t;
}

struct cee_json * cee_json_array(int s) {
  struct cee_array * v = cee_array (s);
  struct cee_tagged * t = cee_tagged (is_array, v);
  return (struct cee_json *)t;
}

struct cee_json * cee_json_object() {
  struct cee_map * m = cee_map ((cee_cmp_fun)strcmp);
  struct cee_tagged * t = cee_tagged (is_object, m);
  return (struct cee_json *)t;
}
...
void cee_json_object_set_bool(struct cee_json * j, char * key, bool b) {
  ...
  if (!o) 
    cee_segfault();
  cee_map_add(o, cee_str("%s", key), cee_json_bool(b));
}
...
void cee_json_object_set_string (struct cee_json * j, char * key, char * str) {
  ...
  if (!o) 
    cee_segfault();
  cee_map_add(o, cee_str("%s", key), cee_json_string(cee_str("%s", str)));
}
...
void cee_json_object_set_number (struct cee_json * j, char * key, double real) {
  ...
  if (!o) 
    cee_segfault();
  cee_map_add(o, cee_str("%s", key), cee_json_number(real));
}

void cee_json_array_append (struct cee_json * j, struct cee_json *v) {
  struct cee_array * o = cee_json_to_array(j);
  if (!o) 
    cee_segfault();
  cee_array_append(o, v);
}
...
void cee_json_array_append_bool (struct cee_json * j, bool b) {
  ...
  if (!o) 
    cee_segfault();
  cee_array_append(o, cee_json_bool(b));
}
...
void cee_json_array_append_string (struct cee_json * j, char * x) {
  ...
  if (!o) 
    cee_segfault();
  cee_array_append(o, cee_json_string(cee_str("%s", x)));
}
